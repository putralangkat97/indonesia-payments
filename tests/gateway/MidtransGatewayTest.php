...
class MidtransGatewayTest extends TestCase
{
        ...
        $this->client
            ...
            ->with(
                $this->callback(function (array $payload) {
                    return $payload["transaction_details"]["order_id"] ===
                        "ORDER-001" &&
            ...
            ->willReturn([
                ...
                "redirect_url" =>
                    "https://app.sandbox.midtrans.com/snap/v3/redirection/snap_token_123",
            ]);
    ...
    public function test_handle_webhook_with_valid_signature(): void
    {
        ...
        $signature = hash(
            "sha512",
            $order_id . $status_code . $gross_amount . $this->server_key,
        );
    ...
    public function test_map_status(
        string $midtrans_status,
        PaymentStatus $expected,
    ): void {
        $this->client->method("getTransactionStatus")->willReturn([
            "order_id" => "ORDER-001",
